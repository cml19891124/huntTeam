...
- (UIView *)contentView {
    if (!_contentView) {
        _contentView = [[UIView alloc] initWithFrame:CGRectMake(0, ScreenHeight-kNavBarHeight, ScreenWith, 220)];
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.alpha = 0;
        // 存放每种情况需要分割的列
        self.columnArray = @[@(1),@(2),@(3),@(4),@(5),@(6),@(2),@(3),@(2),@(2)];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    switch (self.pickerViewMode) {
        case SPDatePickerModeMonthAndDay:
        {
            switch (component) {
                case 0:
                {
...
-(void)setCurrentDate:(NSDate *)currentDate
{
    ...
    switch (self.pickerViewMode) {
        ...
        {
            selectedYear = 2000;
            [self.pickerView selectRow:month inComponent:0 animated:NO];
            [self.pickerView selectRow:day inComponent:1 animated:NO];
            
            [self pickerView:self.pickerView didSelectRow:month inComponent:0];
            [self pickerView:self.pickerView didSelectRow:day inComponent:1];
        }
            break;
        default:
            break;
    ...
    switch (self.pickerViewMode) {
        ...
        case 9:
        {
            switch (component) {
                ...
                {
                    label.textAlignment=NSTextAlignmentLeft;
                    label.text=[NSString stringWithFormat:@"%ld月",(long)row];
                }
                    break;
                case 1:
                {
                    label.textAlignment=NSTextAlignmentRight;
                    label.text=[NSString stringWithFormat:@"%ld日",(long)row];
                }
                    break;
...
- (void)showDateTimePickerView{
    ...
    [UIView animateWithDuration:0.25f animations:^{
        self.alpha = 1;
        self.contentView.frame = CGRectMake(0, ScreenHeight-220-kNavBarHeight, ScreenWith, 220);
