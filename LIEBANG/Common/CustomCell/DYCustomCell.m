...
#import "DYCustomCell.h"

@implementation DYCustomCell
...
- (void)delegateEvent {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(DYCustomCell:event:)]) {
        
        [self.delegate DYCustomCell:self event:self.data];
    }
}
...
- (void)loadContentWithAdapter:(CellDataAdapter *)dataAdapter
                      delegate:(id <DYCustomCellDelegate>)delegate
                     indexPath:(NSIndexPath *)indexPath {
...
@implementation UITableView (DYCustomCell)

- (DYCustomCell *)dequeueReusableCellAndLoadDataWithAdapter:(CellDataAdapter *)adapter indexPath:(NSIndexPath *)indexPath {
    
    DYCustomCell *cell = [self dequeueReusableCellWithIdentifier:adapter.cellReuseIdentifier forIndexPath:indexPath];
    [cell loadContentWithAdapter:adapter delegate:nil tableView:self indexPath:indexPath];
    
    return cell;
}

- (DYCustomCell *)dequeueReusableCellAndLoadDataWithAdapter:(CellDataAdapter *)adapter
                                                 delegate:(id <DYCustomCellDelegate>)delegate
                                                indexPath:(NSIndexPath *)indexPath {
...
- (void)selectedEventAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DYCustomCell *cell = [self cellForRowAtIndexPath:indexPath];
    if ([cell isKindOfClass:[DYCustomCell class]]) {
