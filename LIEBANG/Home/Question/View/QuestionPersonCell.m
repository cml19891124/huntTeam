...
@end
@implementation QuestionPersonCell
...
- (void)setDetailModel:(QuestionOrderDetailModel *)detailModel {
    ...
    if (_detailType == QuestionDetailTypeExpert) {
        if ([detailModel.orderStates intValue] == 1 && [[_sureButton currentTitle] isEqualToString:@"回答"]) {
...
- (void)layoutSubviews
{
    ...
    if (self.detailModel.company.length == 0 && self.detailModel.position.length == 0) {
        ...
        if (nameW > kDeviceWidth - _nameLabel.width - _iconImageView.width - kCurrentWidth(12)) {
            _personLabel.width = self.detailModel.isOccupationOne.intValue == 1?kDeviceWidth-_nameLabel.width - _iconImageView.width - kCurrentWidth(115):kDeviceWidth-_nameLabel.width - _iconImageView.width - kCurrentWidth(82);
        }else{
...
//        _personLabel.hidden = NO;
//        _personLabel.labelWidth = kDeviceWidth-(_nameLabel.right+kCurrentWidth(10))-kCurrentWidth(62);
//        [_personLabel setCompany:@"|   " postion:model.position showIcon:model.isOccupation];
//        _personLabel.left = _nameLabel.right+kCurrentWidth(10);
//
//    }
...
/*
    ...
    _nameLabel.left = _iconImageView.right + kCurrentWidth(25);
...
- (void)createSubViews {
    ...
    WeakSelf;
    _nameLabel = [[NameLabel alloc] initWithFrame:CGRectMake(_iconImageView.right+kCurrentWidth(15), kCurrentWidth(13), kCurrentWidth(45), kCurrentWidth(20))];
