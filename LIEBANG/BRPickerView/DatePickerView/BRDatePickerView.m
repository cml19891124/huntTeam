...
+ (void)showDatePickerWithTitle:(NSString *)title
    ...
    UIButton *sender = [[AppDelegate currentAppDelegate].window viewWithTag:5600];
...
+ (void)showDatePickerWithTitle:(NSString *)title
    ...
    UIButton *sender = [[AppDelegate currentAppDelegate].window viewWithTag:5600];
    [sender setTitle:@"至今" forState:UIControlStateNormal];
}
...
#pragma mark - 因为选择日期大于当前时期闪退，故9.16注释
...
- (void)setupDayArr:(NSInteger)year month:(NSInteger)month {
    ...
    if (self.showType == BRDatePickerModeMD)
    {
        NSInteger startDay = 1;
        NSInteger endDay = [NSDate br_getDaysInYear:2000 month:month];
