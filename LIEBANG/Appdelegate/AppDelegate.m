...
- (BOOL)application:(UIApplication *)application
  ...
  sourceApplication:(NSString *)sourceApplication
    ...
    if ([url.scheme isEqualToString:@"LIEBANGYIQI"])
    {
        if ([url.host isEqualToString:@"safepay"]) {
            ...
            [[AlipaySDK defaultService] processOrderWithPaymentResult:url standbyCallback:^(NSDictionary *resultDic) {
                if (resultDic && [resultDic objectForKey:@"resultStatus"] && ([[resultDic objectForKey:@"resultStatus"] intValue] == 9000)) {
                    ...
                    // 发通知带出支付成功结果
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"ALI_ORDER_PAY_NOTIFICATION" object:resultDic];
                } else {
    ...
    if ([url.scheme isEqualToString:WXappKey] && [url.absoluteString containsString:@"pay"])
    {
        return [WXApi handleOpenURL:url delegate:(id<WXApiDelegate>)self];
    }
    if ([url.scheme isEqualToString:WXappKey] && [url.absoluteString containsString:@"oauth"])
    {
        return  [WXApi handleOpenURL:url delegate:(id<WXApiDelegate>)self];
    }
    ...
    if ([resp isKindOfClass:[PayResp class]]) {
        switch (resp.errCode) {
            case WXSuccess:{
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:@"WX_ORDER_PAY_NOTIFICATION" object:@"success"];
                break;
            }
            default:{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"WX_ORDER_PAY_NOTIFICATION" object:@"fail"];
                break;
            }
        }
    }
        ...
        if (rep.errCode == 0)
        {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"WX_ACCOUNT_LOGIN_RETURN" object:@{@"code":rep.code}];
        }else{
            [[NSNotificationCenter defaultCenter] postNotificationName:@"WX_ACCOUNT_LOGIN_RETURN" object:@{@"code":@"fail"}];
        }
    }
}
