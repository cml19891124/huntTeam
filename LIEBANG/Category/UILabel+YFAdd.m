...
//  LIEBANG
//
//  Created by caominglei on 2019/11/1.
...
CG_INLINE CGFloat
UIEdgeInsetsInLabelGetHorizontalValue(UIEdgeInsets insets) {
    return insets.left + insets.right;
}

/// 获取UIEdgeInsets在垂直方向上的值
CG_INLINE CGFloat
UIEdgeInsetsInLabelGetVerticalValue(UIEdgeInsets insets) {
...
- (CGSize)yf_sizeThatFits:(CGSize)size {
    UIEdgeInsets insets = self.yf_contentInsets;
    size = [self yf_sizeThatFits:CGSizeMake(size.width - UIEdgeInsetsInLabelGetHorizontalValue(insets), size.height-UIEdgeInsetsInLabelGetVerticalValue(insets))];
    size.width += UIEdgeInsetsInLabelGetHorizontalValue(insets);
    size.height += UIEdgeInsetsInLabelGetVerticalValue(insets);
    return size;
}
