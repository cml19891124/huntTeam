...
-(void)createPageControl{
    ...
    _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(0, ZXMainScrollViewHeight-kCurrentWidth(27), kDeviceWidth, kCurrentWidth(27))];
...
- (void)setInfoByCurrentImageIndex:(NSUInteger)currentImageIndex {
    ...
    id model1 = [self.data safeObjectAtIndex:currentImageIndex];
    if ([model1 isKindOfClass:[IndexBanner class]]) {
        IndexBanner *dto = (IndexBanner *)model1;
        [self.centerIV sd_setImageWithURL:[NSURL URLWithString:dto.bannerUrl] placeholderImage:self.placeHolderImage];
    }
    else if ([model1 isKindOfClass:[TopicBanner class]]) {
        TopicBanner *dto = (TopicBanner *)model1;
        [self.centerIV sd_setImageWithURL:[NSURL URLWithString:dto.bannerUrl] placeholderImage:self.placeHolderImage];
    }
//    IndexBanner *model1 = [self.data safeObjectAtIndex:currentImageIndex];
//    if ([self isHttpString:model1.bannerUrl]) {
//        [self.centerIV sd_setImageWithURL:[NSURL URLWithString:model1.bannerUrl] placeholderImage:self.placeHolderImage];
//    } else {
////        self.centerIV.image = model1.bannerUrl;
//    }

    NSInteger leftIndex = (unsigned long)((_currentImageIndex - 1 + self.imgCount) % self.imgCount);
    id model2 = [self.data safeObjectAtIndex:leftIndex];
    if ([model2 isKindOfClass:[IndexBanner class]]) {
        IndexBanner *dto = (IndexBanner *)model2;
        [self.centerIV sd_setImageWithURL:[NSURL URLWithString:dto.bannerUrl] placeholderImage:self.placeHolderImage];
    }
    else if ([model2 isKindOfClass:[TopicBanner class]]) {
        TopicBanner *dto = (TopicBanner *)model2;
        [self.centerIV sd_setImageWithURL:[NSURL URLWithString:dto.bannerUrl] placeholderImage:self.placeHolderImage];
    }
//    NSInteger leftIndex = (unsigned long)((_currentImageIndex - 1 + self.imgCount) % self.imgCount);
//    IndexBanner *model2 = [self.data safeObjectAtIndex:leftIndex];
//    if([self isHttpString:model2.bannerUrl]){
//        [self.leftIV sd_setImageWithURL:[NSURL URLWithString:model2.bannerUrl] placeholderImage:self.placeHolderImage];
//    }else {
////        self.leftIV.image = model2.bannerUrl;
//    }
    
    NSInteger rightIndex = (unsigned long)((_currentImageIndex + 1) % self.imgCount);
    id model3 = [self.data safeObjectAtIndex:rightIndex];
    if ([model3 isKindOfClass:[IndexBanner class]]) {
        IndexBanner *dto = (IndexBanner *)model3;
        [self.centerIV sd_setImageWithURL:[NSURL URLWithString:dto.bannerUrl] placeholderImage:self.placeHolderImage];
    }
    else if ([model3 isKindOfClass:[TopicBanner class]]) {
        TopicBanner *dto = (TopicBanner *)model3;
        [self.centerIV sd_setImageWithURL:[NSURL URLWithString:dto.bannerUrl] placeholderImage:self.placeHolderImage];
    }

//    NSInteger rightIndex = (unsigned long)((_currentImageIndex + 1) % self.imgCount);
//    IndexBanner *model3 = [self.data safeObjectAtIndex:rightIndex];
//    if([self isHttpString:model3.bannerUrl]){
//        [self.rightIV sd_setImageWithURL:[NSURL URLWithString:model3.bannerUrl] placeholderImage:self.placeHolderImage];
//    }else {
////        self.rightIV.image = model3.bannerUrl;
//    }

    _pageControl.currentPage = rightIndex;
    if (_HW3DBannerScrollViewBlock) {
        _HW3DBannerScrollViewBlock(rightIndex);
    }
}
...
-(void)centerTapGes{
    ...
    if (self.clickImageBlock) {
        self.clickImageBlock(rightIndex);
    }
}
